...
"""#*Propuesta 2*
En la siguiente propuesta de prototipo se tiene establecido la lectura por consola de el paso del listado completo de la enfermedades y respoder si corresponde o no el sintoma para brindar un diagnostico 
"""
...
# Lista de sintomas, los cuales serán los insumos para la predicción de la enfermedad, se tomaron en cuenta los sintomas generales de las enfermedades digestivas, y los particulares asociadas a la lista de enfermedades.
sintomas = [ 'Sangrado', 'Distensión', 'Estreñimiento', 'Diarrea', 'Acidez gástrica', 'Incontinencia', 'Náuseas', 'Vómitos', 'Dolor en el abdomen', 'Problemas para tragar', 'Aumento o pérdida de peso', 'Fiebre', 'Color amarillo en la piel y en la parte blanca de los ojos (ictericia)', 'Dolor en la parte superior derecha o media del abdomen durante al menos 30 minutos', 'Heces con sangre', 'Falta de apetito']

#Lista de enfermedades a predecir.
enfermedades = ['Colecistitis', 'Cálculos Biliares', 'Proctitis', 'Hepatitis B']
...
X = []
y = []
# Solicitamos al usuario los síntomas
for sint in sintomas:
    respuesta = input(f"¿Tiene {sint}? (Sí/No): ")
    if respuesta.lower() == 'sí':
        X.append(1)
    else:
        X.append(0)
# Pasamos a dos dimenciones
X = [X]

# Agregar etiqueta para los datos de entrenamiento, no existe
y.append(0)

# Entrenar 
arbol_decision.fit(X, y)

# Predecir la enfermedad basada en los síntomas ingresados por el usuario
prediccion_enfermedad = arbol_decision.predict(X)

# Obtener el diagnóstico médico
diagnostico = enfermedades[prediccion_enfermedad[0]]

print("El diagnóstico médico es:", diagnostico)
...
'''
    ...
    @prediccion: Listas para hacer la predicción.
'''
def pred_3_sintomas(arbol_decision, prediccion):
    # Lista de sintomas que se van a tomar para la predicción.
    x = [[7, 8, 13], [8, 7, 13], [13, 7, 8], [13, 8, 7], [7, 13, 8], [8, 13, 7], []]
    # Lista de enfermedades que se van a tomar para la predicción.
    y =["Colecistitis", "Colecistitis", "Colecistitis", "Colecistitis","Colecistitis", "Colecistitis",]
    # Entrenamiento del modelo.
    arbol_decision = arbol_decision.fit(x,y)
    # Predicción de la enfermedad.
    prediccion_enfermedad = arbol_decision.predict([prediccion])
    # Retornamos la predicción.
    return prediccion_enfermedad